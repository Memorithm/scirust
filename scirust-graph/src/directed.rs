...
impl<E> DirectedGraph<E> {
    ...
    ///
    /// Both Kosaraju passes are iterative so graph depth does not consume the
    /// process call stack on large sparse graphs.
    #[must_use]
    pub fn strongly_connected_components(&self) -> Vec<usize> {
        ...
        for start in 0..self.node_count
        {
            if !seen[start]
            {
                self.finish_from_iterative(start, &mut seen, &mut finish_order);
            }
        }
        ...
        for &start in finish_order.iter().rev()
        {
            ...
            let mut component = self.reverse_component_iterative(start, &mut seen);
    ...
    fn finish_from_iterative(&self, start: usize, seen: &mut [bool], order: &mut Vec<usize>) {
        seen[start] = true;
        let mut stack = vec![(start, 0usize)];

        while let Some((node, next_neighbor)) = stack.last_mut()
        {
            let begin = self.out_offsets[*node];
            let end = self.out_offsets[*node + 1];
            if begin + *next_neighbor < end
            {
                let target = self.edges[begin + *next_neighbor].target;
                *next_neighbor += 1;
                if !seen[target]
                {
                    seen[target] = true;
    ...
    fn reverse_component_iterative(&self, start: usize, seen: &mut [bool]) -> Vec<usize> {
        seen[start] = true;
        ...
        {
            component.push(node);
            let begin = self.in_offsets[node];
            let end = self.in_offsets[node + 1];
            for &edge_index in self.in_edge_indices[begin..end].iter().rev()
            {
...
mod tests {
    ...
    #[test]
    fn strongly_connected_components_handle_deep_graph_without_recursion() {
        let node_count = 20_000usize;
        let edges = (0..node_count - 1)
            .map(|source| edge(source, source + 1, 1))
            .collect();
        let graph =
            DirectedGraph::from_edges(node_count, edges, DirectedGraphOptions::default()).unwrap();
        let labels = graph.strongly_connected_components();
        assert_eq!(labels.len(), node_count);
        assert_eq!(labels[0], 0);
        assert_eq!(labels[node_count - 1], node_count - 1);
    }
