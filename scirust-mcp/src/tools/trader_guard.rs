...
fn parse_strict_row(row: &Value, path: &str) -> Result<(i64, f32, f32, f32, f32, f32), String> {
    if let Some(columns) = row.as_array()
    {
        ...
        let ts_ms = columns[0].as_i64().ok_or_else(|| {
            invalid(
                &format!("{path}[0]"),
                "timestamp must be an i64 millisecond value",
            )
        })?;
        return Ok((
            ts_ms,
    ...
    let ts_ms = object.get("ts").and_then(Value::as_i64).ok_or_else(|| {
        invalid(
            &format!("{path}.ts"),
            "required i64 millisecond timestamp is missing or invalid",
        )
    })?;
    Ok((
        ts_ms,
...
fn finite_f32(value: &Value, path: &str) -> Result<f32, String> {
    ...
    if !narrowed.is_finite()
    {
        return Err(invalid(
            path,
            "is outside the finite f32 range used by scirust-trader",
        ));
    }
    Ok(narrowed)
}
...
fn validate_mock_market_data_args(arguments: &Value) -> Result<(), String> {
    ...
    if let Some(source) = object.get("source")
    {
        ...
        {
            return Err(
                "unsupported_source: trader_market_data currently supports only source='mock'"
                    .to_string(),
            );
        }
    }
...
fn bridge_indicator_params(arguments: &mut Value) -> Result<(), String> {
    ...
    else
    {
        return Err(invalid(
            "$",
            "trader_indicators arguments must be an object",
        ));
    };
    let Some(params) = object.get("params").cloned()
    else
    {
        return Ok(());
    };
