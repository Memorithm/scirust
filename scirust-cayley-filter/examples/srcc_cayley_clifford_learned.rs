//! Supervised train/dev/test subspace-recovery control.
//!
//! All methods receive the same signal/noise cases and are evaluated with
//! the same rejected dimension and loss.
//!
//! The SRCC variant intentionally uses the training noise vectors directly
//! as seeds. Therefore no resonant expansion is expected:
//! `rounds == 0` and `certificates().is_empty()`.
//!
//! This benchmark validates deterministic seed-span recovery and held-out
//! projection quality. It does not measure SRCC transport discovery or
//! resonant-consensus closure.

use scirust_cayley_filter::{
    CayleyProjector, DevelopmentGateDecision, MultiplierCase, NoiseSubspaceProjector,
    SEDENION_DIMENSION, Sedenion, SplitCliffordProjector, analyze_matrix,
    fit_clifford_noise_subspace, left_multiplication_matrix, select_hard_cayley_train_dev,
    squared_norm,
};
...
fn make_split(
    ...
    (0..count)
        .map(|sample| {
            ...
            let noise = mixed_noise(oracle.orthonormal_basis(), family, split, sample);
...
fn fit_srcc_from_training_noise(train: &[Case]) -> Result<SrccProjector, String> {
    let seeds: Vec<Sedenion> = train.iter().map(|case| case.noise).collect();
    ...
    SrccProjector::build(&seeds, &transports, config).map_err(|error| error.to_string())
}
...
fn main() -> Result<(), String> {
    ...
    for family in 0..FAMILY_COUNT
    {
        ...
        println!(
            "# family={family} cayley_decision={:?} cayley_candidates={} srcc_rounds={} srcc_certificates={} cayley_gap={:.17e} clifford_gap={:.17e} srcc_gap={:.17e}",
