...
SciRust est un framework d'apprentissage profond écrit en Rust pur. Il combine une bibliothèque de runtime avec deux attributs de transformation de source réels, #[autodiff] et #[simd]. L'exécution GPU est sélectionnée explicitement via des backends CPU, wgpu et CUDA testés ; SciRust ne revendique pas de macro procédurale automatique Rust-vers-GPU. Le projet est positionné comme un **artefact de recherche**, et non comme un concurrent de production aux frameworks établis (PyTorch, ou en Rust, Burn et candle), qui le dépassent en termes de couverture d'opérateurs, de maturité des noyaux et de diversité matérielle.
...
Ce qui distingue SciRust d'une simple bibliothèque est sa dimension de transpilation ciblée. Les crates scirust-macros et scirust-simd-macros implémentent les attributs #[autodiff] et #[simd]. L'accélération reste un choix explicite du runtime, car l'ancienne macro expérimentale #[gpu] n'effectuait pas de véritable abaissement de noyau. Les calculs numériques CPU sont en Rust pur sans dépendance BLAS obligatoire, ce qui — comme le montre la Section 4 — a rendu la portabilité multi-architecture simple.
