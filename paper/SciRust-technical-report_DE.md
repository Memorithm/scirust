...
**Tarek Zekriti**
Unabhängiger Forscher · zekrititarek@gmail.com
...
SciRust ist ein in reinem Rust geschriebenes Deep-Learning-Framework. Es kombiniert eine Runtime-Bibliothek mit zwei echten Quelltransformationsattributen, #[autodiff] und #[simd]. Die GPU-Ausführung wird explizit über getestete CPU-, wgpu- und CUDA-Backends gewählt; SciRust beansprucht kein automatisches Rust-zu-GPU-Prozedurmakro. Das Projekt ist als **Forschungsartefakt** positioniert, nicht als Produktionskonkurrent zu etablierten Frameworks (PyTorch oder in Rust Burn und candle), die es in Operatorenabdeckung, Kernel-Reife und Hardware-Breite übertreffen.
...
Was SciRust von einer reinen Bibliothek unterscheidet, ist seine fokussierte Transpiler-Dimension. scirust-macros und scirust-simd-macros implementieren #[autodiff] und #[simd]. Beschleunigung bleibt eine explizite Runtime-Wahl, da das frühere experimentelle #[gpu]-Makro kein echtes Kernel-Lowering durchführte. Die CPU-Numerik ist reiner Rust-Code ohne obligatorische BLAS-Abhängigkeit, was – wie Abschnitt 4 zeigt – die architekturübergreifende Portabilität einfach machte.
