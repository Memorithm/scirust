...
fn simplex_diameter(simplex: &[Vec<f64>]) -> f64 {
    ...
    for i in 0..simplex.len() {
        for j in (i + 1)..simplex.len() {
            let dist: f64 = simplex[i].iter().zip(&simplex[j])
                .map(|(a, b)| (a - b).abs())
                .fold(0.0, f64::max);
    ...
    F: Fn(&[f64]) -> f64,
{
    ...
    for j in 0..n {
        let mut v = x0.clone();
        let delta = if x0[j].abs() < 1e-12 { step } else { step * x0[j].abs() };
        v[j] += delta;
        simplex.push(v);
    }

    let mut fvals: Vec<f64> = simplex.iter().map(|v| {
        let val = f(v);
    ...
    for k in 0..tol.max_iter {
        ...
        for &idx in &order[..n] {
            for d in 0..n { centroid[d] += simplex[idx][d]; }
        }
        for d in 0..n { centroid[d] /= n as f64; }

        // Vérifier que centroid ≠ worst (simplex non dégénéré)
        let is_degenerate = centroid.iter().zip(&simplex[worst])
        ...
        if is_degenerate {
            ...
            for j in 0..n {
                ...
                let delta = if best_pt[j].abs() < 1e-12 { step } else { step * best_pt[j].abs() };
                v[j] += delta;
                simplex[j + 1] = v;
            }
            for i in 0..=n {
        ...
        let xr: Vec<f64> = (0..n)
            ...
            .collect();
        for &xrd in &xr { check_finite(xrd, "xr")?; }
        let fr = f(&xr);
        check_finite(fr, "fr")?;
        ...
        if fr < fvals[best] {
            ...
            let xe: Vec<f64> = (0..n)
                ...
                .collect();
            for &xed in &xe { check_finite(xed, "xe")?; }
            let fe = f(&xe);
            check_finite(fe, "fe")?;
            if fe < fr {
            ...
            let xc: Vec<f64> = if fr < fvals[worst] {
                (0..n).map(|d| centroid[d] + rho * (xr[d] - centroid[d])).collect()
            } else {
                (0..n).map(|d| centroid[d] + rho * (simplex[worst][d] - centroid[d])).collect()
            };
            for &xcd in &xc { check_finite(xcd, "xc")?; }
            let fc = f(&xc);
            check_finite(fc, "fc")?;
...
mod tests {
    ...
    fn nelder_quadratic() {
        let s = nelder_mead(
            ...
            vec![0.0, 0.0], 1.0,
            Tolerance { abs: 1e-8, rel: 1e-8, max_iter: 500 },
        ).unwrap();
    ...
    fn nelder_rosenbrock() {
        let s = nelder_mead(
            ...
            vec![-1.2, 1.0], 0.5,
            Tolerance { abs: 1e-8, rel: 1e-8, max_iter: 5000 },
        ).unwrap();
    ...
    fn nelder_non_differentiable() {
        let s = nelder_mead(
            ...
            vec![10.0], 1.0,
            Tolerance { abs: 1e-6, rel: 1e-6, max_iter: 500 },
        ).unwrap();
