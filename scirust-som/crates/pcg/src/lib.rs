...
impl PcgBuilder {
    ...
    fn process_function(&mut self, func: &ast::Function) {
        ...
        let region_node = self
            .pcg
            .add_node(PcgNode::Region(format!("region_{}", func.name)));
        ...
        match stmt
        {
            ...
            ast::Statement::Scope(statements) =>
            {
                ...
                let scope_region = self
                    .pcg
                    .add_node(PcgNode::Region(format!("scope_{}", self.pcg.nodes.len())));
                ...
                for i in 0..self.pcg.nodes.len()
                {
                    if let PcgNode::Variable(_) = &self.pcg.nodes[i]
                    {
                        if self
                            ...
                            .edges
                            .iter()
                            .any(|(f, t, e)| *f == scope_region && *t == i && *e == PcgEdge::Owns)
                        {
            ...
            ast::Statement::Expression(expr) =>
            {
                let dummy = self
                    .pcg
                    .add_node(PcgNode::MemoryLocation("temp".to_string()));
            ...
            ast::Statement::Return(Some(expr)) =>
            {
                let dummy = self
                    .pcg
                    .add_node(PcgNode::MemoryLocation("return".to_string()));
                self.process_expression(dummy, expr);
            },
            _ =>
            {},
...
mod tests {
    ...
    fn test_pcg_move() {
        ...
        let x_idx = pcg
            .nodes
            .iter()
            .position(|n| matches!(n, PcgNode::Variable(name) if name == "x"))
            .unwrap();
        let y_idx = pcg
            .nodes
            .iter()
            .position(|n| matches!(n, PcgNode::Variable(name) if name == "y"))
            .unwrap();

        assert!(
            pcg.edges
                .iter()
                .any(|(f, t, e)| *f == x_idx && *t == y_idx && *e == PcgEdge::Moves)
        );
    }
    ...
    fn test_pcg_borrow() {
        ...
        assert!(
            pcg.edges
                .iter()
                .any(|(f, t, e)| *f == y_idx && *t == x_idx && *e == PcgEdge::Borrows)
        );
    }
    ...
    fn test_pcg_scope_drop() {
        let prog = SomAst::Program(vec![Function {
            ...
            params: vec![],
            body: vec![Statement::Scope(vec![Statement::VarDecl {
                name: "x".to_string(),
                ty: Type::Int,
                init: Some(Expression::Literal(Literal::Int(1))),
            }])],
        ...
        let scope_idx = pcg
            .nodes
            .iter()
            .position(|n| matches!(n, PcgNode::Region(name) if name.starts_with("scope_")))
            .unwrap();

        assert!(
            pcg.edges
                .iter()
                .any(|(f, t, e)| *f == scope_idx && *t == x_idx && *e == PcgEdge::Owns)
        );
        assert!(
            pcg.edges
                .iter()
                .any(|(f, t, e)| *f == scope_idx && *t == x_idx && *e == PcgEdge::Drops)
        );
    }
}
