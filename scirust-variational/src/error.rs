...
pub enum VariationalError {
    ...
    InvalidInterval { start: f32, end: f32 },
    InvalidBoundaryCondition { details: String },
    SingularVelocityHessian { condition_number: f32, tolerance: f32 },
    IllConditionedSystem { condition_number: f32, tolerance: f32 },
    NonFiniteValue { component: &'static str, value: f32 },
    AutodiffFailure { details: String },
    SymbolicFailure { details: String },
    LinearSolveFailure { details: String },
    NonlinearSolveFailure { details: String },
    ConstraintViolation { constraint: String, residual: f32 },
    InfeasibleControlProblem { details: String },
    TrainingFailure { details: String },
...
impl fmt::Display for VariationalError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SingularVelocityHessian { condition_number, tolerance } => {
                write!(f, "singular velocity Hessian: condition number {condition_number:.2e} exceeds tolerance {tolerance:.2e}")
            }
            Self::IllConditionedSystem { condition_number, tolerance } => {
                write!(f, "ill-conditioned system: condition number {condition_number:.2e} exceeds tolerance {tolerance:.2e}")
            }
            ...
            Self::ConstraintViolation { constraint, residual } => {
                write!(f, "constraint violation '{constraint}': residual {residual:.2e}")
            }
