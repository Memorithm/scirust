...
impl ActionProblem {
    ...
    pub fn with_initial_condition(mut self, index: usize, condition: BoundaryCondition) -> Result<Self> {
    ...
    pub fn with_terminal_condition(mut self, index: usize, condition: BoundaryCondition) -> Result<Self> {
        if index >= self.num_coordinates {
            return Err(VariationalError::DimensionMismatch {
                expected: self.num_coordinates,
                got: index + 1,
