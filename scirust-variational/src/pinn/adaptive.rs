...
impl AdaptiveSampler {
        ...
        R: Fn(f32) -> f32,
    {
        ...
        let mut points = CollocationPoints::from_random_uniform(
            domain,
            config.initial_points,
            config.seed,
        );
        let mut history = AdaptiveRefinementHistory {
            rounds: Vec::new(),
        };
        ...
        for round in 0..config.refinement_rounds {
            ...
            let mean_res =
                residuals.iter().sum::<f32>() / residuals.len().max(1) as f32;
...
mod tests {
    ...
    fn test_adaptive_refinement() {
        ...
        let (points, history) =
            AdaptiveSampler::refine(&domain, |x| (x - 0.5).abs(), &config);
