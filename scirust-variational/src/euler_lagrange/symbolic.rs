use scirust_symbolic::{diff, simplify, Expr};

use crate::error::{Result, VariationalError};
use super::{substitute, ELEquation, ELDerivation};
...
impl SymbolicEulerLagrange {
    pub fn derive(
        ...
        for (_i, (q_name, dq_name)) in coordinates.iter().zip(velocities.iter()).enumerate() {
    ...
    pub fn derive_from_lagrangian_string(
        ...
        let velocities: Vec<String> = coordinates
            .iter()
            .map(|c| format!("{}_dot", c))
            .collect();
...
mod tests {
    ...
    fn test_free_particle() {
        let m = 1.0;
        let lagrangian = Expr::Const(0.5) * Expr::Const(m) * Expr::Pow(Box::new(Expr::Var("dq".to_string())), Box::new(Expr::Const(2.0)));
    ...
    fn test_harmonic_oscillator() {
        ...
        let k = 1.0;
        let lagrangian = Expr::Const(0.5) * Expr::Const(m) * Expr::Pow(Box::new(Expr::Var("dq".to_string())), Box::new(Expr::Const(2.0)))
            - Expr::Const(0.5) * Expr::Const(k) * Expr::Pow(Box::new(Expr::Var("q".to_string())), Box::new(Expr::Const(2.0)));
        let coords = vec!["q".to_string()];
        let vels = vec!["dq".to_string()];
        let result = SymbolicEulerLagrange::derive(&lagrangian, &coords, &vels, None).unwrap();
        let eq = &result.equations[0];
    ...
    fn test_simple_pendulum() {
        ...
        let l = 1.0;
        let lagrangian =
            Expr::Const(0.5) * Expr::Const(m) * Expr::Const(l * l) * Expr::Pow(Box::new(Expr::Var("dtheta".to_string())), Box::new(Expr::Const(2.0)))
                - Expr::Const(m) * Expr::Const(g) * Expr::Const(l)
    ...
    fn test_coupled_oscillator() {
        ...
        let k2 = 1.0;
        let k3 = 1.0;
        let lagrangian = Expr::Const(0.5) * Expr::Const(m1) * Expr::Pow(Box::new(Expr::Var("dq1".to_string())), Box::new(Expr::Const(2.0)))
            + Expr::Const(0.5) * Expr::Const(m2) * Expr::Pow(Box::new(Expr::Var("dq2".to_string())), Box::new(Expr::Const(2.0)))
            - Expr::Const(0.5) * Expr::Const(k1) * Expr::Pow(Box::new(Expr::Var("q1".to_string())), Box::new(Expr::Const(2.0)))
            - Expr::Const(0.5) * Expr::Const(k2) * Expr::Pow(Box::new(Expr::Var("q2".to_string()) - Expr::Var("q1".to_string())), Box::new(Expr::Const(2.0)))
            - Expr::Const(0.5) * Expr::Const(k3) * Expr::Pow(Box::new(Expr::Var("q2".to_string())), Box::new(Expr::Const(2.0)));
    ...
    fn test_time_dependent_lagrangian() {
        let lagrangian = Expr::Sin(Box::new(Expr::Var("t".to_string()))) * Expr::Pow(Box::new(Expr::Var("q".to_string())), Box::new(Expr::Const(2.0)))
            + Expr::Const(0.5) * Expr::Pow(Box::new(Expr::Var("dq".to_string())), Box::new(Expr::Const(2.0)));
        let coords = vec!["q".to_string()];
        let vels = vec!["dq".to_string()];
        let result =
            SymbolicEulerLagrange::derive(&lagrangian, &coords, &vels, Some("t")).unwrap();
        assert_eq!(result.num_coordinates(), 1);
    ...
    fn test_euler_lagrange_identity() {
        let lagrangian = Expr::Pow(Box::new(Expr::Var("dq".to_string())), Box::new(Expr::Const(2.0)));
        let coords = vec!["q".to_string()];
        let vels = vec!["dq".to_string()];
        let result = SymbolicEulerLagrange::derive(&lagrangian, &coords, &vels, None).unwrap();
        let eq = &result.equations[0];
    ...
    fn test_display_euler_lagrange() {
        let lagrangian = Expr::Const(0.5) * Expr::Pow(Box::new(Expr::Var("dq".to_string())), Box::new(Expr::Const(2.0)));
        let coords = vec!["q".to_string()];
        let vels = vec!["dq".to_string()];
        let result = SymbolicEulerLagrange::derive(&lagrangian, &coords, &vels, None).unwrap();
