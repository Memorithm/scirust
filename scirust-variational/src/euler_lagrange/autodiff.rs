        ...
        F: for<'a> Fn(&'a NdTape, &'a [NdVar<'a>], &'a [NdVar<'a>], Option<NdVar<'a>>) -> NdVar<'a>,
    {
        if q.len() != self.ndim || dq.len() != self.ndim {
            return Err(VariationalError::DimensionMismatch {
                expected: self.ndim,
                got: if q.len() != self.ndim { q.len() } else { dq.len() },
        ...
        let dL_ddq: Vec<f32> = grads[self.ndim..2 * self.ndim].iter().map(|g| g.data[0]).collect();
...
mod tests {
    ...
    fn harmonic_lagrangian<'a>(tape: &'a NdTape, q: &'a [NdVar<'a>], dq: &'a [NdVar<'a>], _t: Option<NdVar<'a>>) -> NdVar<'a> {
    ...
    fn test_harmonic_acceleration() {
        ...
        let q = vec![1.0];
        let dq = vec![0.0];
        let result = el.compute_acceleration(&harmonic_lagrangian, &q, &dq, None).unwrap();
        assert_relative_eq!(result.acceleration[0], -1.0, epsilon = 0.1);
    }

    fn singular_lagrangian<'a>(tape: &'a NdTape, q: &'a [NdVar<'a>], _dq: &'a [NdVar<'a>], _t: Option<NdVar<'a>>) -> NdVar<'a> {
        let one = tape.input(TensorND::new(vec![1.0], vec![1, 1]));
        q[0].mul(q[0]).mul(one)
    }
