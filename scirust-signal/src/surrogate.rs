...
pub fn phase_randomized_surrogate(signal: &[f64], seed: u64) -> Result<Vec<f64>, SurrogateError> {
    ...
    // DC and Nyquist are self-conjugate for an even-length real signal and are
    // intentionally left untouched.  Randomize only one bin of every remaining
    // conjugate pair, then construct its partner explicitly.
    for k in 1..nyquist {
