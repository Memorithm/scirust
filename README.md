...
- **Certifiable & reproducible** — **Interval Bound Propagation** gives *provable* output bounds and robustness certificates for ReLU MLPs (`scirust certify`); **conformal prediction** gives distribution-free prediction sets with a *guaranteed* coverage level (`scirust conformal`); **order-independent floating-point reductions** are bit-identical regardless of thread count; **Wanda** pruning and **SmoothQuant** extend the deterministic int8 path.
