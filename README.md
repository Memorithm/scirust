...
- **Pattern detection** (`scirust-vision`, `scirust-audio`, `scirust-graph`, `scirust-sequential`, `scirust-multivariate`, `scirust-unsupervised`, `scirust-seasonal`, `scirust-nlp-advanced`) — computer vision (CNN, HOG, LBP, Canny, Otsu, NMS, plus **optronics / precision-optics imaging** — Gaussian **and diffraction-limited Airy PSFs** (with the Rayleigh resolution limit `1.22·λ/D`), MTF with the MTF50 resolution metric, **Richardson–Lucy (spatial) and Wiener (frequency-domain) deconvolution** for EO/IR image restoration, ray optics: ABCD ray-transfer matrices with Gaussian-beam propagation via the complex q-parameter for optical-train design, and **image-domain small-target CFAR detection** — the EO/IR analogue of the radar CFAR, pulling small hot targets out of a spatially varying thermal background (local guard/training-ring statistics, intensity-weighted sub-pixel centroids) to feed the tracking chain from an IR focal plane), audio (MFCC, chroma, pitch YIN, onset detection), graph patterns (subgraph isomorphism, motif discovery, community detection, betweenness), sequential (HMM, CRF, DTW, KMP/Boyer-Moore), multivariate (PCA, ICA, K-Means++, MDS, CCA), unsupervised (autoencoder, isolation forest, DBSCAN, LOF, GMM), seasonal (STL, ACF/PACF, Mann-Kendall, CUSUM), NLP (NER, LDA, TextRank, MinHash, NaiveBayes).
