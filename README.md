...
- **Pattern detection** (`scirust-vision`, `scirust-audio`, `scirust-graph`, `scirust-sequential`, `scirust-multivariate`, `scirust-unsupervised`, `scirust-seasonal`, `scirust-nlp-advanced`) — computer vision (CNN, HOG, LBP, Canny, Otsu, NMS, plus **optronics / precision-optics imaging** — Gaussian **and diffraction-limited Airy PSFs** (with the Rayleigh resolution limit `1.22·λ/D`), MTF with the MTF50 resolution metric, **Richardson–Lucy (spatial) and Wiener (frequency-domain) deconvolution** for EO/IR image restoration, ray optics: ABCD ray-transfer matrices with Gaussian-beam propagation via the complex q-parameter for optical-train design, **image-domain small-target CFAR detection** — the EO/IR analogue of the radar CFAR, pulling small hot targets out of a spatially varying thermal background (local guard/training-ring statistics, intensity-weighted sub-pixel centroids) to feed the tracking chain from an IR focal plane, and **thermal-infrared radiometry & sensor sensitivity** — Planck's law, Stefan–Boltzmann `σT⁴` and Wien's peak-wavelength law, band-integrated thermal contrast, the **NETD** (noise-equivalent temperature difference) and **MRTD** (minimum resolvable temperature difference, the NETD-over-MTF trade-off) metrics that spec a thermal imager, **Beer–Lambert atmospheric transmission & the EO/IR range budget** — path transmittance `e^{−αR}`, extinction from meteorological visibility (Koschmieder), the contrast-transmission law, and the target ΔT required to detect through an attenuating path at range, and **atmospheric optical turbulence & adaptive-optics metrics** — the **Fried parameter** `r₀ = (0.423·k²·Cn²·L)^(−3/5)` (∝ λ^(6/5)), the **seeing angle** `0.98·λ/r₀` that supersedes the diffraction limit `λ/D` once `D > r₀`, the **Strehl ratio** `[1+(D/r₀)^(5/3)]^(−6/5)`, the **Greenwood frequency** `0.426·v/r₀` (adaptive-optics loop bandwidth), the correction **degrees of freedom** `(D/r₀)²`, and the **Rytov scintillation variance** `1.23·Cn²·k^(7/6)·L^(11/6)` — the physics that sizes an EO/IR telescope, laser designator, or AO system against a `Cn²` turbulence profile, and a batch of further EO/IR primitives — **two-point non-uniformity correction** (IR focal-plane gain/offset calibration and residual-FPN metric), **LIDAR time-of-flight & CW phase ranging**, **sub-pixel centroiding** (intensity-weighted / thresholded / windowed centre of gravity for EO/IR pointing), and **Zernike wavefront aberrations** (defocus/astigmatism/coma/spherical, RMS wavefront error, and the Maréchal Strehl `exp(−(2πσ)²)`)), audio (MFCC, chroma, pitch YIN, onset detection), graph patterns (subgraph isomorphism, motif discovery, community detection, betweenness), sequential (HMM, CRF, DTW, KMP/Boyer-Moore), multivariate (PCA, ICA, K-Means++, MDS, CCA), unsupervised (autoencoder, isolation forest, DBSCAN, LOF, GMM), seasonal (STL, ACF/PACF, Mann-Kendall, CUSUM), NLP (NER, LDA, TextRank, MinHash, NaiveBayes).
...
- **Relativity, fractional calculus & non-local (memory) dynamics** (`scirust-relativity`, `scirust-fractional`, `scirust-nonlocal-relativity`) — a research pillar complementing the memoryless plants in `scirust-sim`: fixed-background **general-relativity kinematics** (metrics, Christoffel symbols / `Connection<D>`, test-particle geodesic worldlines); a **Caputo L1 fractional derivative** (`caputo_l1_uniform`) — a reusable fractional-order primitive; and an explicitly **experimental non-local (memory) extension** of worldline dynamics where the ordinary state equation carries a projected Caputo velocity-history force `F_memory = −κ·P·Caputo_α[u]`. The non-local layer keeps a **complete-history oracle** (`O(D·N²)`) with an opt-in **bounded short-memory** approximation (`O(D·N·W)`), semi-implicit Euler as the reference path plus a **Heun PECE** predictor-corrector, self-convergence and Schwarzschild-exterior diagnostics, and — the pieces that make the memory *geometric* — **parallel-transported memory** (`DiscreteConnectionTransport`, a Heun approximation of `dV^μ/dλ = −Γ^μ_{αβ}u^α V^β` along the accepted worldline) that cuts cross-chart disagreement by orders of magnitude versus raw coordinate memory (and shrinks under refinement), an **affine-vs-proper-time** parameterization with typed rejection of non-timelike states, a **Cartesian-vs-cylindrical Minkowski chart-covariance** comparison, and an opt-in **curvature-modulated memory** hook (`SchwarzschildKretschmannModulator`, a phenomenological `q = 1 + β·L⁴·K` reweighting by the Kretschmann scalar `K = 48M²/r⁶`, bypassed bit-for-bit at `β = 0`). It does not modify the Einstein field equations, stress-energy, or matter-sourced curvature, and claims no empirical validation of new physics.
- **Prospective dynamic-information analysis** (`scirust-tdi`) — an exact, deterministic toolkit (ported from the *TDI* research project) asking whether the **structure of accessible futures** carries predictive information that scalar summaries such as Shannon entropy do not preserve: **exact finite-state dynamics** with arbitrary-precision **rational** probabilities (no floating-point rounding), **future-structure descriptors** (future-block / branching-path / branching-state distributions and the intervention-conditioned **`distribution_overlap`**), and **honest baselines** (Shannon block entropy, an orbital baseline, perturbation-recovery analysis). Its 40 exact tests carry over verbatim. Honest about its own limits: within the tested synthetic families it challenges entropy-only sufficiency, but TDI-1's signal was fully subsumed by the orbital baseline (incremental gain `0`) and it establishes no universal law or cross-size invariance — the deterministic, exact machinery and honest evaluation discipline are what transfer. The companion `scirust-tdi-bench` re-homes the reproducible evaluation harness (deterministic scans and counterexample search, untouched holdouts, ridge models, and a **deterministic bootstrap** for confidence intervals, with frozen preregistration constants) as ten deterministic benchmark binaries.
...
- **2-D field simulation** (`scirust-itd`) — the platform's first spatial-field / PDE-flavoured kernel, complementing `scirust-sim`'s ODE plants: deterministic finite-difference **curl/vorticity** and **gradient** operators (reproducing NumPy's `gradient(edge_order=2)` for uniform *and* non-uniform rectilinear axes, finite or periodic boundaries), **2-D trapezoidal spatial integration** on non-uniform grids, a five-component **structural signature** (heterogeneity, localization, roughness, sign-mixing, temporal deformation) with a normalized-weighted score, a **curvature-weighted rotational-intensity** driver `⟨ω²·e^{L²κ}⟩` reduced to interval-integrated indices, and a **semi-Lagrangian transport-compensated** temporal mode (periodic advection of the vorticity by midpoint/RK4 back-tracing with bilinear or 16-point cubic periodic interpolation, so the temporal term measures genuine deformation rather than mere transport). A pure-Rust, zero-dependency, `#![forbid(unsafe_code)]` port of the ITD research simulator, every operator, transport scheme and the published 161×161/401-step scenario indices validated against the reference implementation as an oracle to ~1e-9 (plus analytic invariants: an irrotational field yields zero intensity, a rigid rotation yields constant vorticity, zero-velocity transport is the identity). A faithful numerical port, not a physical claim.
...
scirust-itd/           2-D field-simulation core: finite-difference curl/gradient (finite & periodic, uniform & non-uniform grids), 2-D trapezoidal integration, five-component structural signature, curvature-weighted rotational intensity, semi-Lagrangian transport-compensated temporal mode (oracle-validated port of the ITD research simulator)
scirust-relativity/    Fixed-background GR kinematics: metrics, Christoffel symbols (Connection<D>), test-particle geodesic worldlines
scirust-fractional/    Fractional calculus: Caputo L1 derivative (caputo_l1_uniform)
scirust-nonlocal-relativity/  Experimental non-local (memory) worldline dynamics: projected Caputo velocity-history force, complete-history oracle + bounded short-memory, Heun PECE, parallel-transported memory (DiscreteConnectionTransport), affine/proper-time, chart-covariance, curvature-modulated memory
scirust-tdi/           Prospective dynamic-information analysis (ported from TDI): exact finite-state dynamics with rational probabilities, future-structure descriptors (future-block/branching distributions, intervention-conditioned distribution overlap), Shannon/orbital baselines, perturbation-recovery analysis
scirust-tdi-bench/     Reproducible TDI benchmarks: deterministic scans, counterexamples, untouched holdouts, ridge models, deterministic bootstrap CIs, frozen preregistration constants (ten benchmark binaries over scirust-tdi)
