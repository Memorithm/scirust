...
/// Deterministic seed derivation stream for sampler substreams.
///
/// The generator uses a SplitMix64-style bijective mixer. [`Self::seed_for`]
/// is random-access and depends only on the root seed, trial id and lane, while
/// [`Self::next_seed`] provides a reproducible sequential stream. The core
/// deliberately defines seed derivation separately from any particular random
/// number generator so samplers can use their own RNG implementation without
/// losing cross-run stream identity.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl SeedStream {
    /// Create a deterministic seed stream from one root seed.
    ...
    /// Return the number of sequential seeds already emitted.
    pub const fn position(self) -> u64 {
        self.counter
    }

    /// Derive a stable seed for one trial and algorithm-defined lane.
    ///
    /// This operation does not mutate the sequential counter and therefore can
    /// be used safely for random-access worker/subalgorithm streams.
    pub fn seed_for(self, trial: TrialId, lane: u64) -> u64 {
        mix_seed(
            self.root
                ^ mix_seed(trial.get().wrapping_mul(0x9e37_79b9_7f4a_7c15))
                ^ lane.wrapping_mul(0xd1b5_4a32_d192_ed03),
        )
    }

    /// Emit the next deterministic sequential seed.
...
/// Append-only state transition emitted by [`Study`].
///
/// Events contain only typed optimizer state and are independent of a concrete
/// persistence format. Persistent backends can encode this stream as binary,
/// JSON, a journal or a database log while retaining deterministic replay.
#[derive(Debug, Clone, PartialEq)]
pub enum StudyEvent {
    /// A new monotone trial identifier was reserved.
    TrialReserved {
        /// Reserved trial.
        trial: TrialId,
    },
    /// A reserved trial transitioned to Running.
    TrialStarted {
        /// Started trial.
        trial: TrialId,
    },
    /// One parameter value was assigned or overwritten.
    ParameterAssigned {
        /// Trial receiving the value.
        trial: TrialId,
    ...
    /// A trial completed with objective values.
    TrialCompleted {
        /// Completed trial.
        trial: TrialId,
        /// Objective values in configured objective order.
        values: Vec<f64>,
    },
    /// A trial was pruned.
    TrialPruned {
...
/// Error returned while deterministically replaying a study event stream.
#[derive(Debug, Clone, PartialEq)]
pub enum ReplayError {
    /// Study construction failed before replay started.
    Study(StudyError),
    /// A replayed reservation did not match the next monotone identifier.
    ReservationMismatch {
        /// Event index in the supplied stream.
        event_index: usize,
        /// Identifier encoded in the event.
        expected: TrialId,
        /// Identifier generated by the reconstructed store.
        found: TrialId,
    },
    /// Event could not be applied to the reconstructed trial state.
    Trial {
        /// Event index in the supplied stream.
        event_index: usize,
        /// Underlying lifecycle/storage error.
        source: TrialError,
    },
    /// Event was semantically idempotent or otherwise failed to append exactly
    /// one matching canonical event during replay.
    NonCanonicalEvent {
        /// Event index in the supplied stream.
        event_index: usize,
    },
...
impl fmt::Display for ReplayError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self
        {
            Self::Study(source) => write!(f, "study construction failed: {source}"),
            Self::ReservationMismatch {
                ...
                f,
                "event {event_index} reserved {expected:?}, reconstructed {found:?}"
            ),
            Self::Trial {
                event_index,
                source,
...
impl std::error::Error for ReplayError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self
        {
            Self::Study(source) => Some(source),
            Self::Trial { source, .. } => Some(source),
            Self::ReservationMismatch { .. } | Self::NonCanonicalEvent { .. } => None,
        }
    }
}
...
/// Immutable in-memory snapshot of a study.
///
/// The representation is intentionally opaque: callers can inspect the search
/// space, trial store and event cursor, then restore it through
/// [`Study::from_snapshot`]. A persistent backend may serialize an equivalent
/// representation without coupling the core crate to a serialization library.
#[derive(Debug, Clone)]
...
impl StudySnapshot {
    /// Return the compiled search space captured by the snapshot.
    pub const fn search_space(&self) -> &SearchSpace {
        &self.space
    }

    /// Return the captured trial store.
    pub const fn trials(&self) -> &TrialStore {
        &self.store
    }

    /// Number of events incorporated into the snapshot.
...
impl Study {
    ...
    /// Reconstruct a study by replaying canonical events in order.
    ///
    /// Replay checks monotone reservation identity and requires every supplied
    /// event to reproduce exactly one canonical event. Duplicate idempotent
    /// transitions are therefore rejected rather than silently normalized.
    pub fn replay(
        space: SearchSpace,
        objective_count: usize,
        events: &[StudyEvent],
    ) -> Result<Self, ReplayError> {
        let mut study = Self::new(space, objective_count).map_err(ReplayError::Study)?;
        for (event_index, event) in events.iter().enumerate()
        {
            let before = study.events.len();
            match event
            {
                StudyEvent::TrialReserved { trial } =>
                {
                    let found = study.reserve().map_err(|source| ReplayError::Trial {
                        event_index,
                        source,
                    })?;
                    if found != *trial
                    {
                        return Err(ReplayError::ReservationMismatch {
                            event_index,
                            expected: *trial,
                            found,
                        });
                ...
                {
                    study.start(*trial).map_err(|source| ReplayError::Trial {
                        event_index,
                        source,
                    })?;
                },
                ...
                {
                    study
                        .tell(*trial, TrialOutcome::Complete(values.clone()))
            ...
            if study.events.len() != before + 1 || study.events.last() != Some(event)
            {
                return Err(ReplayError::NonCanonicalEvent { event_index });
            }
        }
        Ok(study)
    }

    /// Restore an opaque snapshot without replaying its already-incorporated
    /// prefix.
    pub fn from_snapshot(snapshot: StudySnapshot) -> Self {
        Self {
    ...
    /// Capture the current in-memory state and canonical event prefix.
    ...
    /// Return the canonical append-only event stream.
    pub fn events(&self) -> &[StudyEvent] {
        &self.events
    }

    /// Return events at or after a watermark cursor.
    ///
    /// Returning `None` means the cursor is beyond the current event stream.
    /// An exact end cursor returns an empty slice.
    pub fn events_since(&self, cursor: usize) -> Option<&[StudyEvent]> {
    ...
    pub fn reserve(&mut self) -> Result<TrialId, TrialError> {
        let trial = self.store.reserve()?;
        self.events.push(StudyEvent::TrialReserved { trial });
        Ok(trial)
    }
    ...
    fn fail_reserved(&mut self, trial: TrialId) {
        if let Some(row) = self.store.row(trial)
            && !matches!(
                self.store.states[row],
                TrialState::Complete | TrialState::Pruned | TrialState::Failed
            )
        {
            self.store.states[row] = TrialState::Failed;
            self.events.push(StudyEvent::TrialFailed { trial });
        }
    }
    ...
    pub fn start(&mut self, trial: TrialId) -> Result<(), TrialError> {
        ...
        match self.store.states[row]
        {
            TrialState::Reserved =>
            {
                ...
                self.events.push(StudyEvent::TrialStarted { trial });
                Ok(())
            },
    ...
    pub fn set_param(
        ...
        self.events.push(StudyEvent::ParameterAssigned {
            trial,
            param,
            value,
        });
    ...
    pub fn tell(&mut self, trial: TrialId, outcome: TrialOutcome) -> Result<(), TrialError> {
        ...
        if !already_terminal
        {
            ...
            {
                TrialOutcome::Complete(values) => StudyEvent::TrialCompleted { trial, values },
                TrialOutcome::Pruned => StudyEvent::TrialPruned { trial },
                TrialOutcome::Failed => StudyEvent::TrialFailed { trial },
            };
...
mod tests {
    ...
    #[test]
    fn seed_stream_is_reproducible_and_random_access() {
        let mut a = SeedStream::new(0x5eed);
        let mut b = SeedStream::new(0x5eed);
        let seq_a: Vec<_> = (0..8).map(|_| a.next_seed()).collect();
        let seq_b: Vec<_> = (0..8).map(|_| b.next_seed()).collect();
        assert_eq!(seq_a, seq_b);
        assert_eq!(a.position(), 8);
        assert_eq!(
            SeedStream::new(0x5eed).seed_for(TrialId::new(42), 3),
            SeedStream::new(0x5eed).seed_for(TrialId::new(42), 3)
        );
        assert_ne!(
            SeedStream::new(0x5eed).seed_for(TrialId::new(42), 3),
            SeedStream::new(0x5eed).seed_for(TrialId::new(43), 3)
        );
    }

    #[test]
    fn event_log_records_idempotent_transitions_once() {
        let mut study = Study::new(conditional_space(), 1).unwrap();
        let trial = study.reserve().unwrap();
        study
            .set_param(trial, ParamId::new(0), ParamValue::Categorical(0))
        ...
        study.start(trial).unwrap();
        study
            .tell(trial, TrialOutcome::Complete(vec![0.25]))
            .unwrap();
        study
            .tell(trial, TrialOutcome::Complete(vec![0.25]))
            .unwrap();
        ...
        assert_eq!(
            ...
            &[
                ...
                StudyEvent::ParameterAssigned {
                    trial,
                    param: ParamId::new(0),
                    value: ParamValue::Categorical(0),
                },
                StudyEvent::ParameterAssigned {
                    trial,
                    param: ParamId::new(2),
                    value: ParamValue::Float(1e-3),
                },
                StudyEvent::TrialStarted { trial },
    ...
    #[test]
    fn replay_reconstructs_trial_store_and_events() {
        let mut original = Study::new(conditional_space(), 1).unwrap();
        let mut sampler = PendingAwareSampler {
            seen_running: Vec::new(),
            choice: 1,
        };
        ...
        for trial in original.trials().trial_ids()
        {
            ...
            {
                assert_eq!(
                    replayed.trials().param_value(trial, param.id),
                    original.trials().param_value(trial, param.id)
                );
            }
            assert_eq!(
                replayed.trials().outcome(trial),
                original.trials().outcome(trial)
            );
        }
    }

    #[test]
    fn replay_rejects_duplicate_idempotent_event() {
        let trial = TrialId::new(0);
        let events = vec![
            StudyEvent::TrialReserved { trial },
            StudyEvent::TrialStarted { trial },
            StudyEvent::TrialStarted { trial },
        ...
        assert_eq!(
            Study::replay(conditional_space(), 1, &events),
            Err(ReplayError::NonCanonicalEvent { event_index: 2 })
        );
    }

    #[test]
    fn snapshot_restores_state_and_delta_cursor() {
        let mut study = Study::new(conditional_space(), 1).unwrap();
        let trial = study.reserve().unwrap();
        study
            .set_param(trial, ParamId::new(0), ParamValue::Categorical(0))
        ...
        let cursor = snapshot.event_count();
        let mut restored = Study::from_snapshot(snapshot);
        restored
            .tell(trial, TrialOutcome::Complete(vec![0.75]))
            .unwrap();

        assert_eq!(restored.trials().state(trial), Some(TrialState::Complete));
        assert_eq!(
            restored.events_since(cursor),
            Some(
                [StudyEvent::TrialCompleted {
                    trial,
                    values: vec![0.75],
