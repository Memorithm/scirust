...
TSHF 研究プログラム（`TSHF_RESEARCH_2026-07-16.md`）に由来する 3 つの補完：

- **`denoise::vst`** — *信号依存*ノイズ向け：**バイアス補正付き**逆変換を備えた分散安定化変換（Anscombe + Poisson 向けの Mäkitalo-Foi による厳密な不偏逆変換、Poisson-ガウス混合センサーモデル `x = gain·p + n` 向けの **GAT** とその 2013 年の厳密な逆変換、乗法性ノイズ向けの符号付き log + Duan の smearing、符号付き平方根、Box-Cox）。保守的なセレクタ `detect_noise_model`（デフォルト = 恒等変換）は `denoise_auto` の条件付き前/後ステップとして組み込まれています。実測値：+5.0 dB（低カウント Poisson）、+4.9 dB（乗法性 30 %）、+1.4〜+3.0 dB（Poisson-ガウス混合）、穏やかな条件では ±0 dB — 損失は決して生じません。文書化された既知の制限：高速な搬送波（平方根が生成する高調波を内部のノイズ除去器が削ってしまう。実測 ≈ −1 dB）— VST はゆっくり変化する強度を対象とします。**2-D 画像**版（`vst_denoise2d`）は `scirust_vision::denoise` にあり、完全な実験プロトコル（レポートの §9）は `cargo run -p scirust-signal --example vst_protocol` で再実行できます。
- **`denoise::multichannel`** — チャネルを実際に結合する演算子：`wiener_spatial`（チャネル間ジョイント Wiener、相関のあるソースではチャネル毎の制限版に対して +2.5〜+3.7 dB）と `vector_median`（Astola 1990 のリファレンス。チャネル毎メディアンに対する実測の*不利な*判定とともに保存 — `phase2_gate_report()` を参照）。
- **`denoise::compand`** — 有界ソフトクリッピング（`soft_clip`、`soft_clip_robust`、tanh/atan/softsign）。表示およびロバストな特徴量向けで、**設計上、逆変換を持ちません**：飽和変換の逆変換はノイズを ×10-×100 に増幅します（TSHF レポート、E2/E4）。
