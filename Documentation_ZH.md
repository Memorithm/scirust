...
源自 TSHF 研究计划（`TSHF_RESEARCH_2026-07-16.md`）的三个补充模块：

- **`denoise::vst`** —— 针对*信号相关*噪声：带**偏差校正**逆变换的方差稳定变换（Anscombe + 面向 Poisson 的 Mäkitalo-Foi 精确无偏逆变换；面向 Poisson-高斯混合传感器模型 `x = gain·p + n` 的 **GAT** 及其 2013 年精确逆变换；面向乘性噪声的带符号 log + Duan smearing；带符号平方根；Box-Cox）。保守的选择器 `detect_noise_model`（默认 = 恒等变换）作为 `denoise_auto` 的条件式前/后处理步骤接入。实测：+5.0 dB（低计数 Poisson）、+4.9 dB（乘性 30 %）、+1.4 至 +3.0 dB（Poisson-高斯混合）、温和场景下 ±0 dB——从不带来损失。已记录的已知限制：快速载波（平方根会产生谐波，被内部去噪器削除；实测 ≈ −1 dB）——VST 面向缓变强度。**2-D 图像**对应实现（`vst_denoise2d`）位于 `scirust_vision::denoise`；完整实验方案（报告 §9）可用 `cargo run -p scirust-signal --example vst_protocol` 重放。
- **`denoise::multichannel`** —— 真正耦合各通道的算子：`wiener_spatial`（跨通道联合 Wiener，在相关源上相比其逐通道受限版本提升 +2.5 至 +3.7 dB）与 `vector_median`（Astola 1990 参考实现，连同其相对逐通道中值的实测*不利*裁定一并保留——见 `phase2_gate_report()`）。
- **`denoise::compand`** —— 有界软削波（`soft_clip`、`soft_clip_robust`；tanh/atan/softsign），用于显示与稳健特征，**设计上不提供逆变换**：对饱和变换求逆会将噪声放大 ×10-×100（TSHF 报告，E2/E4）。
