...
impl fmt::Display for ThermoError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self
        {
            ...
            ThermoError::Negative { name, value } =>
            {
                write!(f, "argument `{name}` must be >= 0, got {value}")
            },
            ...
            ThermoError::NoConvergence { what } =>
            {
...
pub(crate) fn finite(name: &'static str, v: f64) -> Result<f64, ThermoError> {
    if v.is_finite()
    {
        Ok(v)
    }
    ...
    {
        Err(ThermoError::NonFinite { name })
    }
}

/// Require `v` to be finite and strictly positive.
pub(crate) fn positive(name: &'static str, v: f64) -> Result<f64, ThermoError> {
    finite(name, v)?;
    if v > 0.0
    {
        Ok(v)
    ...
    {
        Err(ThermoError::NonPositive { name, value: v })
    }
}

/// Require `v` to be finite and non-negative.
pub(crate) fn non_negative(name: &'static str, v: f64) -> Result<f64, ThermoError> {
    finite(name, v)?;
    if v >= 0.0
    {
        Ok(v)
    ...
    {
        Err(ThermoError::Negative { name, value: v })
    }
}

/// Require `v` to be finite and inside `[min, max]`.
pub(crate) fn in_range(name: &'static str, v: f64, min: f64, max: f64) -> Result<f64, ThermoError> {
    finite(name, v)?;
