...
validation s'étend à deux autres domaines : (1) **vibrations de roulement réelles**
(CWRU) — un défaut de bague externe atteint le gate impulsif mais reste classé
non-`Impulsive` grâce au veto de périodicité, préservant la signature de défaut
(`tests/real_data_vibration.rs`, exemple `classify_real_bearing`) ; (2) **parole
bruitée réelle** (VoiceBank+DEMAND) — cartographie honnête d'une limite : le toolkit
généraliste n'améliore pas la parole sur le SNR de forme d'onde (`denoise_auto`
sur-traite), la leçon étant d'appeler `stft_wiener_auto` directement
(`tests/real_data_audio.rs`, exemple `denoise_real_speech`).
