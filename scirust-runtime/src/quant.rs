...
impl QModel {
    ...
    pub fn from_bytes(b: &[u8]) -> io::Result<QModel> {
        if b.len() < 8 || &b[0..4] != QMAGIC {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "mauvais magic QSR1"));
        }
        let mut p = 4usize;
        ...
        for _ in 0..n {
            ...
            match tag {
                0 => {
                    ...
                    layers.push(QLayer::Linear(QLinear { in_f, out_f, s_in, relu_after, scales, w_q, bias_i32 }));
                }
                other => return Err(io::Error::new(io::ErrorKind::InvalidData, format!("tag couche inconnu {other}"))),
            }
        }
        Ok(QModel { layers })
    }
...
mod tests {
    ...
    fn qmodel_roundtrip_and_deterministic() {
        let m = QModel { layers: vec![
            QLayer::Linear(QLinear { in_f: 3, out_f: 2, s_in: 0.01, relu_after: true,
                scales: vec![0.002, 0.003], w_q: vec![1, -2, 3, 4, -5, 6], bias_i32: vec![10, -20] }),
            QLayer::Linear(QLinear { in_f: 2, out_f: 2, s_in: 0.05, relu_after: false,
                scales: vec![0.004, 0.001], w_q: vec![7, -8, 9, 10], bias_i32: vec![0, 5] }),
        ]};
