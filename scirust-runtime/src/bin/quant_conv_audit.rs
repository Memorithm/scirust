...
fn fakequant_per_row(t: &Tensor) -> Tensor {
    ...
    for r in 0..rows {
        let mut ma = 0.0f32;
        for c in 0..cols { ma = ma.max(t.data[r * cols + c].abs()); }
...
fn fakequant_per_col(t: &Tensor) -> Tensor {
    ...
    for c in 0..cols {
        let mut ma = 0.0f32;
        for r in 0..rows { ma = ma.max(t.data[r * cols + c].abs()); }
        scales[c] = if ma == 0.0 { 1.0 } else { ma / 127.0 };
    }
    let mut out = vec![0.0f32; rows * cols];
    for r in 0..rows {
        for c in 0..cols {
            let q = (t.data[r * cols + c] / scales[c]).round().clamp(-128.0, 127.0);
...
fn main() {
    ...
    for i in 0..n {
        if argmax(&lo[i * 10..i * 10 + 10]) == argmax(&lq[i * 10..i * 10 + 10]) { agree += 1; }
    }
    ...
    println!();
    println!("=== AUDIT b1 : fake-quant per-channel CNN (synthetique, batch={}) ===", n);
    println!("fp f32 oracle  : {:#018x}  (attendu 0x1381e4b51d0eeba4)", fp_orig);
    println!("fp poids quant : {:#018x}", fp_q);
    println!("Erreur logits  : max_abs={:.3e}  max_rel={:.3e}", max_abs, max_rel);
