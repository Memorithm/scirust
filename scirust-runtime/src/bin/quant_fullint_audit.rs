...
fn main() {
    ...
    println!();
    println!("=== AUDIT FULL-INT a2 (requant point-fixe entier, MNIST test {}) ===", total);
    println!("Accuracy full-int : {:.2}% ({}/{})", correct as f32 / total as f32 * 100.0, correct, total);
    println!("  (f32 97.73% ; int8 dyn 97.74% ; int8 statique a1 97.71%)");
    println!("Empreinte logits full-int : {:#018x}", fp);
    println!("  hot path inter-couches 100% entier (i64 mult+shift), zero flottant; dequant final f32 sur 10 sorties.");
}
