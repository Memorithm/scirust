...
fn main() {
    ...
    let model = QModel { layers: vec![
        QLayer::Linear(QLinear { in_f: 784, out_f: 256, s_in, relu_after: true, scales: w1s, w_q: w1q, bias_i32: b1_i32 }),
        QLayer::Linear(QLinear { in_f: 256, out_f: 10, s_in: s_hid, relu_after: false, scales: w2s, w_q: w2q, bias_i32: b2_i32 }),
    ]};

    let path = "/tmp/mnist_mlp_lib.qsrt";
    let buf = model.save(path).expect("save");
    let hash = fnv_bytes(&buf);
    let srt1 = std::fs::metadata("mnist_mlp.srt").map(|m| m.len()).unwrap_or(0);
    ...
    println!();
    println!("=== AUDIT c2 : QModel via API lib (MNIST test {}) ===", total);
    println!("Artefact QSR1 (lib) : {} octets  hash {:#018x}", buf.len(), hash);
    println!("SRT1 f32            : {} octets  -> {:.2}x", srt1, srt1 as f64 / buf.len() as f64);
    println!("Accuracy (QModel::infer depuis fichier) : {:.2}% ({}/{})", correct as f32 / total as f32 * 100.0, correct, total);
    println!("Empreinte logits : {:#018x}  (attendu 0xa9b9a102c7cea67b)", fp);
    println!("=> promotion lib fidele au prototype : {}", if fp == 0xa9b9a102c7cea67b { "OUI (bit-pour-bit)" } else { "NON" });
}
