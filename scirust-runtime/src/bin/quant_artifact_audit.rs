...
fn write_qsrt(layers: &[QLinear], path: &str) -> std::io::Result<Vec<u8>> {
    ...
    for l in layers {
        ...
        for &s in &l.scales { b.extend_from_slice(&s.to_le_bytes()); }
        b.extend_from_slice(&(l.w_q.len() as u32).to_le_bytes());
        for &q in &l.w_q { b.push(q as u8); }
        b.extend_from_slice(&(l.bias_i32.len() as u32).to_le_bytes());
        for &x in &l.bias_i32 { b.extend_from_slice(&x.to_le_bytes()); }
    }
    std::fs::write(path, &b)?;
    Ok(b)
}
...
fn rf32(b: &[u8], p: &mut usize) -> f32 { let v = f32::from_le_bytes(b[*p..*p + 4].try_into().unwrap()); *p += 4; v }
fn ri32(b: &[u8], p: &mut usize) -> i32 { let v = i32::from_le_bytes(b[*p..*p + 4].try_into().unwrap()); *p += 4; v }
...
fn infer(layers: &[QLinear], input: &[f32], batch: usize) -> Vec<f32> {
    ...
    for li in 0..layers.len() {
        ...
        for bi in 0..batch {
            for o in 0..l.out_f {
                let a = acc[bi * l.out_f + o] + l.bias_i32[o];
                let (m, sh) = quantize_multiplier((l.s_in as f64 * l.scales[o] as f64) / s_out as f64);
                let mut r = requant(a, m, sh);
                if l.relu_after && r < 0 { r = 0; }
                if r > 127 { r = 127; }
                if r < -128 { r = -128; }
...
fn argmax(row: &[f32]) -> usize {
    let mut bi = 0usize; let mut best = row[0];
    for (i, &v) in row.iter().enumerate().skip(1) { if v > best { best = v; bi = i; } }
    bi
}
fn linear_f32(x: &[f32], batch: usize, in_f: usize, w: &[f32], b: &[f32], out_f: usize) -> Vec<f32> {
    let mut o = vec![0.0f32; batch * out_f];
    for bi in 0..batch { for oj in 0..out_f {
        let mut s = 0.0f32; for i in 0..in_f { s += x[bi * in_f + i] * w[i * out_f + oj]; }
...
fn main() {
    ...
    let test = MnistDataset::load_idx(format!("{}/t10k-images-idx3-ubyte", dir), format!("{}/t10k-labels-idx1-ubyte", dir)).expect("test");
    let train = MnistDataset::load_idx(format!("{}/train-images-idx3-ubyte", dir), format!("{}/train-labels-idx1-ubyte", dir)).expect("train");

    let sd = load_weights("mnist_mlp.srt").expect("load_weights");
    let (mut w1, mut b1, mut w2, mut b2) = (None, None, None, None);
    for (_k, t) in &sd { match t.shape() {
        (784, 256) => w1 = Some(t.clone()), (1, 256) => b1 = Some(t.clone()),
        (256, 10) => w2 = Some(t.clone()), (1, 10) => b2 = Some(t.clone()), _ => {} } }
    ...
    let layers = vec![
        QLinear { in_f: 784, out_f: 256, s_in, relu_after: true, scales: w1s, w_q: w1q, bias_i32: b1_i32 },
        QLinear { in_f: 256, out_f: 10, s_in: s_hid, relu_after: false, scales: w2s, w_q: w2q, bias_i32: b2_i32 },
    ];

    let path = "/tmp/mnist_mlp.qsrt";
    let buf = write_qsrt(&layers, path).expect("write_qsrt");
    let hash = fnv_bytes(&buf);
    let srt1_size = std::fs::metadata("mnist_mlp.srt").map(|m| m.len()).unwrap_or(0);
    ...
    println!();
    println!("=== AUDIT c1 : artefact quantifie QSR1 (MNIST test {}) ===", total);
    println!("Artefact QSR1 : {} octets  hash {:#018x}", buf.len(), hash);
    println!("SRT1 f32      : {} octets  -> reduction {:.2}x", srt1_size, srt1_size as f64 / buf.len() as f64);
    println!("Accuracy (rejeu depuis artefact) : {:.2}% ({}/{})", correct as f32 / total as f32 * 100.0, correct, total);
    println!("Empreinte logits : {:#018x}  (attendu a2 0xa9b9a102c7cea67b)", fp);
}
