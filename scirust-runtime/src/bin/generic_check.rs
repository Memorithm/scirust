...
fn main() {
    ...
    let parsed =
        parse_manifest(&std::fs::read_to_string("cnn.manifest").unwrap()).expect("parse cnn");
    let mut gen = build_model(&parsed);
    gen.load_state_dict(&load_weights("cnn.srt").expect("load cnn"))
        .expect("lsd cnn");
    ...
    assert_eq!(
        f_ref, f_gen,
        "reconstruction CNN par manifeste NON equivalente"
    );
    ...
    let parsed_mlp =
        parse_manifest(&std::fs::read_to_string("mnist.manifest").unwrap()).expect("parse mlp");
    ...
    println!(
        "  empreinte  : {:#018x}  (cible 0xc96d25fa658f5611 : {})",
        fp,
        fp == 0xc96d25fa658f5611
    );
    assert_eq!(
        fp, 0xc96d25fa658f5611,
        "MLP reconstruit ne reproduit pas l'empreinte entrainee"
    );
