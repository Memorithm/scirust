...
fn main() {
    ...
    let model = QModel {
        layers: vec![
            lin(&mut r, d0, d1, 0.02, true),
            lin(&mut r, d1, d2, 0.03, false),
        ],
    };
    let bytes = model.to_bytes();
    let input: Vec<f32> = (0..b * d0)
        .map(|_| (r.n() % 2000) as f32 / 1000.0 - 1.0)
        .collect();
    ...
    let m = scirust_edge::infer(
        &bytes, &input, b, &mut act_a, &mut act_b, &mut acc, &mut out,
    )
    .expect("edge infer");
    let edge_out = &out[..m];
    ...
    println!(
        "bit-identique std vs no_std : {}",
        if same { "OUI" } else { "NON" }
    );
    ...
    println!(
        "scratch RAM total : {} o   (act {} o x2 + acc {} o + out {} o)",
        cert.scratch_ram_bytes, cert.act_bytes_each, cert.acc_bytes, cert.out_bytes
    );
    println!("MAC entiers (exact, deterministe) : {}", cert.mac_count);
    println!("couches {}   dim sortie {}", cert.layers, cert.out_dim);

    let ok_exact = scirust_edge::infer(
        &bytes, &input, b, &mut act_a, &mut act_b, &mut acc, &mut out,
    )
    .is_ok();
    let too_small = scirust_edge::infer(
        &bytes,
        &input,
        b,
        &mut act_a[..cert.act_bytes_each - 1],
        &mut act_b,
        &mut acc,
        &mut out,
    );
    ...
    println!(
        "borne RAM verifiee : suffisante={}  serree(-1o -> BufferTooSmall)={}",
        ok_exact, tight
    );

    let mac_check = (b * d0 * d1 + b * d1 * d2) as u64;
    println!(
        "MAC attendu (b*d0*d1 + b*d1*d2) = {}  -> {}",
        mac_check,
        if mac_check == cert.mac_count
        {
            "concorde"
        }
        else
