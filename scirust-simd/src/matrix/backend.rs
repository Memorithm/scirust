...
pub trait SimdBackend: Send + Sync {
    ...
    fn sgemm_f32(&self, alpha: f32, a: MatrixView<f32>, b: MatrixView<f32>, beta: f32, c: MatrixViewMut<f32>);
...
impl SimdBackend for ScalarBackend {
    ...
    fn sgemm_f32(&self, _alpha: f32, _a: MatrixView<f32>, _b: MatrixView<f32>, _beta: f32, _c: MatrixViewMut<f32>) {
        // Naive triple-loop GEMM — correct but slow
    }

    fn relu_f32(&self, v: &mut [f32]) {
        for x in v { *x = x.max(0.0); }
    }
}
...
impl SimdBackend for PortableSimdBackend {
    fn name(&self) -> &'static str { "portable_simd" }
    ...
    fn sgemv_f32(&self, a: f32, m: MatrixView<f32>, x: &[f32], b: f32, y: &mut [f32]) { ScalarBackend.sgemv_f32(a, m, x, b, y); }
    fn sgemm_f32(&self, a: f32, ma: MatrixView<f32>, mb: MatrixView<f32>, b: f32, mc: MatrixViewMut<f32>) { ScalarBackend.sgemm_f32(a, ma, mb, b, mc); }
