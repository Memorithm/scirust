...
/// # Examples
///
/// Availability can be queried without performing any work:
///
/// ```
...
/// ```
///
/// The CPU backend returns a vector with the input length:
///
/// ```
/// use scirust_core::compute_backend::{ComputeBackend, CpuFallback};
///
...
pub trait ComputeBackend {
    ...
    ///
    /// `CpuFallback` always returns `true`.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ```
    ///
    /// The query is deterministic and has no side effects:
    ///
    /// ```
    /// use scirust_core::compute_backend::{ComputeBackend, CpuFallback};
    /// let backend = CpuFallback;
    ...
    /// # Examples
    ///
    /// An identity kernel preserves the data:
    ///
    /// ```
    /// use scirust_core::compute_backend::{ComputeBackend, CpuFallback};
...
/// ```
///
/// It requires no initialization or device selection:
///
/// ```
/// use scirust_core::compute_backend::{ComputeBackend, CpuFallback};
...
/// function presently returns `Ok`. The `Result` preserves the backend factory
/// contract for future implementations without hiding initialization failures.
///
/// # Examples
///
/// ```
...
/// ```
///
/// The returned backend can execute a convolution immediately:
///
/// ```
/// use scirust_core::compute_backend::get_backend;
/// let backend = get_backend().unwrap();
