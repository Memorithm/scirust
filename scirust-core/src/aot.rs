...
//! little-endian `f32` parameter stream. It does not compile or execute the generated
//! source, validate model quality, or provide a serialization format with versioning.
...
/// bytes. The generated source defines `IN_DIM`, `OUT_DIM`, a `StaticModel` containing
/// fixed arrays, and a const-generic batched `forward` method.
///
/// The function only generates source text; it does not invoke `rustc`, execute the
/// model, or validate that adjacent layer dimensions are mutually compatible. Trailing
/// bytes after the parameters required by `layers` are ignored.
///
/// # Panics
///
