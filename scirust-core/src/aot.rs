...
//! little-endian `f32` parameter stream. Parameter values are emitted through
//! `f32::from_bits`, preserving every accepted IEEE-754 bit pattern exactly. The
//! generator does not compile or execute the generated source, validate model quality,
//! or provide a serialization format with versioning.
...
fn rust_f32_literal(value: f32) -> String {
...
/// bytes. The first linear layer, if any, defines `IN_DIM`, so leading ReLU layers act
/// element-wise over that declared input width. The generated source defines `IN_DIM`,
/// `OUT_DIM`, a `StaticModel` containing fixed arrays, and a const-generic batched
/// `forward` method. Every parameter is emitted as `f32::from_bits`, including subnormal,
/// infinity and NaN payload bit patterns.
///
/// The function only generates source text; it does not invoke `rustc`, execute the
/// model, or validate that adjacent linear layer dimensions are mutually compatible.
/// Trailing bytes after the parameters required by `layers` are ignored.
///
/// # Panics
///
...
pub fn generate_static_pipeline(layers: &[LayerSpec], weights_bytes: &[u8]) -> String {
    ...
    // The first Linear layer declares the external feature width. Leading
