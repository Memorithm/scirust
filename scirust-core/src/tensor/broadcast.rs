pub fn broadcast_add(a: &[f32], b: &[f32]) -> Vec<f32> {
    ...
    assert_eq!(a.len(), b.len(), "broadcast_add requires same-length slices");
    a.iter().zip(b).map(|(x,y)| x + y).collect()
}
pub fn broadcast_mul(a: &[f32], b: &[f32]) -> Vec<f32> {
    assert_eq!(a.len(), b.len(), "broadcast_mul requires same-length slices");
    a.iter().zip(b).map(|(x,y)| x * y).collect()
}
...
pub fn broadcast_get(data: &[f32], shape: &[usize], idx: &[usize]) -> f32 {
    ...
    for (i, (&s, &ix)) in shape.iter().zip(idx).rev().enumerate() {
        flat_idx += ix * stride;
        stride *= s;
    }
    data[flat_idx]
