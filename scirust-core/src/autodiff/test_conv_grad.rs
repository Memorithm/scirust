...
mod tests {
    ...
    fn test_conv2d_gradient_correctness() {
        ...
        for (i, &g) in grad_x.data.iter().enumerate() {
            assert!((g - expected_grad[i]).abs() < 1e-5, "Grad mismatch at index {}: got {}, expected {}", i, g, expected_grad[i]);
        }
    }
    ...
    fn test_conv2d_forward_value_correctness() {
        ...
        for &v in val.data.iter() {
            assert!((v - 10.0).abs() < 1e-5, "Forward value mismatch: got {}, expected 10.0", v);
        }
