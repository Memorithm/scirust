...
mod tests {
    ...
    fn encoder_state_dict_round_trip() {
        let mut rng = PcgEngine::new(0);
        let enc1 = TransformerEncoder::new(
            2, 8, 2, 16, false, &KaimingNormal, &Zeros, &mut rng);
        let sd = enc1.state_dict();

        let mut rng2 = PcgEngine::new(99);
        let mut enc2 = TransformerEncoder::new(
            2, 8, 2, 16, false, &Zeros, &Zeros, &mut rng2);
        enc2.load_state_dict(&sd).unwrap();

        assert_eq!(enc2.blocks[0].mha.w_q.weight.data, enc1.blocks[0].mha.w_q.weight.data);
