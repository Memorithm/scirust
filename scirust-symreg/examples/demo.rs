...
fn show(name: &str, vars: &[&str], front: &[(usize, f64, Expr)]) {
    ...
    for (s, m, e) in front { println!("  {:>6} {:>11.2e}   {}", s, m, e); }
    println!();
}
...
fn main() {
    ...
    show("f(x,y) = x*y + sin(x)", &["x", "y"], &discover(&data, &["x", "y"], &[1, 2, 3], 200, 22, 35, 25));

    let mut data2 = vec![]; let mut x: f64 = -4.0;
    while x <= 4.0 + 1e-9 { data2.push((vec![x], x / (1.0 + x * x))); x += 0.2; }
    show("f(x) = x / (1 + x^2)", &["x"], &discover(&data2, &["x"], &[1, 2, 3], 200, 22, 35, 25));
}
