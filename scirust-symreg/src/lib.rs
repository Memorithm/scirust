...
use scirust_symbolic::{Expr, diff, eval, simplify};
...
fn map_children(e: &Expr, f: &dyn Fn(&Expr) -> Expr) -> Expr {
    match e
    {
        ...
        Sqrt(a) => Sqrt(b(f(a))),
        Abs(a) => Abs(b(f(a))),
        Add(a, c) => Add(b(f(a)), b(f(c))),
        Sub(a, c) => Sub(b(f(a)), b(f(c))),
        Mul(a, c) => Mul(b(f(a)), b(f(c))),
        Div(a, c) => Div(b(f(a)), b(f(c))),
        Pow(a, c) => Pow(b(f(a)), b(f(c))),
    }
}
fn replace_at(e: &Expr, target: usize, counter: &Cell<usize>, sub: &Expr) -> Expr {
    let here = counter.get();
    counter.set(here + 1);
    if here == target
    {
        return sub.clone();
...
fn subtree_at(e: &Expr, target: usize, counter: &Cell<usize>) -> Option<Expr> {
    ...
    {
        Const(_) | Var(_) => None,
        Neg(a) | Sin(a) | Cos(a) | Exp(a) | Ln(a) | Sqrt(a) | Abs(a) =>
        {
            subtree_at(a, target, counter)
        },
...
fn substitute(e: &Expr, names: &[String], vals: &[f64]) -> Expr {
    if let Var(v) = e
    {
        if let Some(p) = names.iter().position(|n| n == v)
        {
            return Const(vals[p]);
        }
    }
...
pub fn fit_constants(
    ...
    if params.is_empty()
    {
        ...
        for (inp, y) in data
        {
            ...
            for (kk, nm) in inputs.iter().enumerate()
            {
                vars.insert(nm.to_string(), inp[kk]);
            }
    ...
    while it < max_iters
    {
        ...
        for (inp, y) in data
        {
            ...
            for (nm, &val) in params.iter().zip(theta.iter())
            {
                vars.insert(nm.to_string(), val);
            }
            let r = eval(expr, &vars).unwrap_or(f64::NAN) - y;
            sse += r * r;
...
fn gen_tree(rng: &mut Rng, depth: usize, maxd: usize, inputs: &[&str]) -> Expr {
    ...
    if term
    {
        if rng.f64() < 0.6
        {
            Var(inputs[rng.range(inputs.len())].to_string())
        }
        else
        {
            Const(rng.f64() * 4.0 - 2.0)
        }
    }
    else
    {
        ...
        {
            0 => Add(
                b(gen_tree(rng, depth + 1, maxd, inputs)),
                b(gen_tree(rng, depth + 1, maxd, inputs)),
            ),
            1 => Sub(
                b(gen_tree(rng, depth + 1, maxd, inputs)),
...
fn crossover(rng: &mut Rng, x: &Expr, y: &Expr) -> Expr {
    let ix = rng.range(size(x));
    let iy = rng.range(size(y));
    let cy = Cell::new(0);
    let sub = subtree_at(y, iy, &cy).unwrap();
    let cx = Cell::new(0);
    replace_at(x, ix, &cx, &sub)
}
fn mutate(rng: &mut Rng, x: &Expr, inputs: &[&str]) -> Expr {
    let ix = rng.range(size(x));
    let fresh = gen_tree(rng, 0, 2, inputs);
    let cx = Cell::new(0);
    replace_at(x, ix, &cx, &fresh)
}
fn tournament(rng: &mut Rng, scores: &[f64], k: usize) -> usize {
    let mut bi = rng.range(scores.len());
    for _ in 1..k
    {
        let j = rng.range(scores.len());
...
fn pareto_insert(arch: &mut Vec<(usize, f64, Expr)>, s: usize, m: f64, e: &Expr) {
    ...
    if arch.iter().any(|&(s2, m2, _)| dominates((s2, m2), (s, m)))
    {
        return;
    }
    if arch
        .iter()
        .any(|&(s2, m2, _)| s2 == s && (m2 - m).abs() < 1e-15)
    {
        return;
    }
...
fn gp(
    ...
    for _ in 0..gens
    {
        ...
        while next.len() < pop_size
        {
            ...
            {
                let pb = tournament(rng, &scores, 4);
                crossover(rng, &pop[pa], &pop[pb])
            }
            else
            {
                pop[pa].clone()
...
mod tests {
    ...
    fn fit_constants_convex() {
        ...
        while x <= 3.0 + 1e-9
        {
            data.push((vec![x], ta * x * x + tb * x + tc));
            x += 0.2;
        }
        let (theta, mse, _) = fit_constants(
            &expr,
            &["a", "b", "c"],
            &data,
            &["x"],
    ...
    fn rational_appears_parsimonious_on_front() {
        ...
        while x <= 4.0 + 1e-9
        {
            data.push((vec![x], x / (1.0 + x * x)));
            x += 0.2;
        }
        let front = discover(&data, &["x"], &[1, 2, 3], 200, 22, 35, 25);
        assert!(
            front.iter().any(|&(s, m, _)| s <= 8 && m < 1e-3),
