...
impl MissionRequest {
    /// Validates request fields before feasibility analysis or approval.
    ///
    ...
    pub fn validate(&self) -> Result<(), MissionValidationError> {
        ...
        {
            return err("mission_id", "must be non-empty");
        }
        finite_positive("capital", self.capital)?;
        finite(
            "objective.target_net_profit",
            self.objective.target_net_profit,
        )?;
        if self.objective.horizon_ms == 0
        {
            return err("objective.horizon_ms", "must be positive");
        }
        if let Some(value) = self.objective.preferred_net_profit_per_round_trip
        {
            finite("objective.preferred_net_profit_per_round_trip", value)?;
        }
        fraction(
...
impl MissionContract {
    /// Validates approval state and live-submit permission in addition to the request.
    ///
...
mod tests {
    ...
    fn request(mode: MissionMode) -> MissionRequest {
        MissionRequest {
            ...
            objective: ProfitObjective {
                ...
                preferred_net_profit_per_round_trip: Some(1.0),
            },
            risk: RiskBudget {
    ...
    fn proposed_request_is_not_an_approved_contract() {
        let c = MissionContract {
            request: request(MissionMode::Paper),
